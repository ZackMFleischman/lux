...
function ParameterRow({ definition, applied, owner, available, client, controller, onError }: {
  ...
  async function change(next: number) {
    if (!available || !owner || !Number.isFinite(next) || next < definition.min || next > definition.max) return;
    const pending = {};
    intent.current = pending;
    setValue(next); setText(String(next)); onError(null);
    try { await controller.setParameters({ [definition.id]: next }); }
    catch (error) {
  ...
  return <div className="parameter-row">
    <label className="parameter-label" id={`parameter-${definition.id}`}><span>{definition.label}{definition.unit ? ` (${definition.unit})` : ''}</span><output>{available ? value.toFixed(2) : '—'}</output></label>
    <Slider aria-label={definition.label} min={definition.min} max={definition.max}
    ...
    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
      <TextField size="small" type="number" value={text} disabled={!available}
        slotProps={{ htmlInput: { 'aria-label': `${definition.label} value`, min: definition.min, max: definition.max, step: definition.step ?? 'any' } }}
        onFocus={() => { editing.current = true; }} onChange={event => { editing.current = true; setText(event.target.value); }}
    ...
    <p className="hint">{available ? `Applied value: ${applied.toFixed(2)}` : 'Connect an authoring runtime to change controls.'}</p>
  </div>;
}
export function ParameterInspector({ runtime, available, client, controller, onError }: {
  ...
  return <>{state.controlSchema.length === 0 && <p className="hint">This visual defines no live controls.</p>}
    {state.controlSchema.map(definition => <ParameterRow key={`${owner}:${definition.id}`} definition={definition}
      applied={state.controls[definition.id]!} owner={owner} available={available} client={client} controller={controller} onError={onError} />)}
