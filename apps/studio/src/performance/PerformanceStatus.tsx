...
export const PerformanceStatus=memo(function PerformanceStatus({performance:value}:{performance?:PerformanceSnapshot}) {
 ...
 const incomplete=worker&&(worker.cpuCall.validity==='incomplete'||(worker.gpu.timestampQueryEnabled&&worker.gpu.validity==='incomplete')||worker.intervalLostRecords>0||worker.intervalInvalidRecords>0);
 const detail=worker?[
  `Completed worker frames: ${worker.produced.sampleCount}; duration samples: ${worker.retainedRecords}; lost: ${worker.lostRecords}; invalid: ${worker.invalidRecords}.`,
  ...
  `GPU samples ${worker.gpu.sampleCount}/${worker.gpu.expectedCount}; failed ${worker.gpu.failedSamples}; dropped ${worker.gpu.droppedSamples}; pending ${worker.gpu.pendingSamples}.`,
  'Routine observations only; hardware budgets are not evaluated.',
 ...
 return <div aria-label="Live performance" title={detail} style={{display:'flex',flexWrap:'wrap',gap:'4px 14px',fontSize:12,fontVariantNumeric:'tabular-nums',color:'inherit'}}>
  ...
  <span>CPU calls {duration(worker?.cpuCall)}</span>
  <span>GPU passes {worker?.gpu.timestampQueryEnabled?duration(worker.gpu):'unavailable'}</span>
