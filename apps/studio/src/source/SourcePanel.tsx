...
export function SourcePanel({ workspace, readOnly, onSave, onCompositionChange, diagnosticTarget, diagnostics = [] }: {
  ...
  return <section className="source-workspace" aria-label="Source workspace" onKeyDownCapture={event => {
    ...
    <div className="source-columns"><div className="source-files">
      ...
      <nav aria-label="Source files">{Object.keys(snapshot.source.files).map(path => <Button key={path} title={path} aria-label={`Open ${path}`} aria-current={snapshot.selectedFile === path ? 'page' : undefined}
        onClick={() => workspace.openFile(path)}><span className="file-path">{path}</span>{path === snapshot.source.entry && <span className="file-marker">Entry</span>}{snapshot.dirtyFiles.includes(path) && <span aria-label="Unsaved">*</span>}</Button>)}</nav>
      {adding && <form onKeyDown={event => { if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); setAdding(false); setNewPath(''); setError(''); addButton.current?.focus(); } }}
        onSubmit={event => { event.preventDefault(); if (!locked) {
          if (attempt(() => workspace.addFile(newPath, ''))) { setNewPath(''); setAdding(false); }
        } }}>
      ...
      <div className="source-editing"><Tabs ref={tabs} value={snapshot.selectedFile || false} onChange={(_, path: string) => workspace.openFile(path)} variant="scrollable" scrollButtons="auto" aria-label="Open source files">
