...
function exact(input, fields, versionKey, identityKey) {
  ...
  if (versioned && ![2,3,4].includes(raw[versionKey])) throw violation(`Unsupported ${versionKey}`);
  const allowed = [...fields,...(versioned ? [versionKey,'assets','assetSetHash'] : []),...(raw[versionKey]>=3 ? ['controls','controlSchemaHash'] : []),...(raw[versionKey]===4 ? ['executionModel','component','componentMetadataHash',...(versionKey==='linkedVersion'?['sdkVersion']:[])] : []),...(Object.hasOwn(raw,identityKey)?[identityKey]:[])];
...
export function artifactBody(input) {
  const a=exact(input,artifactFields,'artifactVersion','bundleHash'), versioned=a.artifactVersion>=2;
  requireHash(a.sourceHash);
  if(typeof a.entry !== 'string' || a.sdkVersion !== (a.artifactVersion===4 ? '0.3.0' : a.artifactVersion===3 ? '0.2.0' : '0.1.0') || a.compilerVersion !== '7.0.2') throw violation('Unsupported compiler or SDK identity');
  ...
  if(a.artifactVersion===4) return {...body,artifactVersion:4,assets:a.assets,assetSetHash:a.assetSetHash,...componentFields(a)};
...
function componentFields(a) {
  if(a.sdkVersion!=='0.3.0'||a.executionModel!=='single-image-source-v1')throw violation('Unsupported component SDK or execution model');
  requireHash(a.componentMetadataHash);
  const component=normalizeComponentMetadata(a.component),parameters=parameterFields(a);
  if(canonicalControlSchemaJson(component.controls)!==canonicalControlSchemaJson(parameters.controls))throw violation('Component control projection mismatch');
  return {sdkVersion:a.sdkVersion,executionModel:a.executionModel,component,componentMetadataHash:a.componentMetadataHash,...parameters};
}
...
export function linkedBody(input) {
  ...
  if(a.linkedVersion===4) return {linkedVersion:4,...body,assets:a.assets,assetSetHash:a.assetSetHash,...componentFields(a)};
...
export async function verifyArtifact(input,hashBytes) {
  ...
  if(body.artifactVersion===4 && await hashBytes(new TextEncoder().encode(canonicalComponentMetadataJson(body.component)))!==body.componentMetadataHash) throw violation('Component metadata hash mismatch');
...
export async function verifyLinked(input,hashBytes,expectedArtifact) {
  ...
  if(body.linkedVersion===4 && await hashBytes(new TextEncoder().encode(canonicalComponentMetadataJson(body.component)))!==body.componentMetadataHash) throw violation('Component metadata hash mismatch');
  if(body.linkedVersion===4 && expectedArtifact && (body.sdkVersion!==expectedArtifact.sdkVersion||body.executionModel!==expectedArtifact.executionModel||body.componentMetadataHash!==expectedArtifact.componentMetadataHash||canonicalComponentMetadataJson(body.component)!==canonicalComponentMetadataJson(expectedArtifact.component))) throw violation('Linked component artifact identity mismatch');
