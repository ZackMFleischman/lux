...
export function validateProbeEvidence(producer, receiver, release) {
  ...
  if (!summary || summary.closed !== true || summary.failed !== false || summary.webgpuReady !== true ||
      summary.held !== 0 || summary.uncertain !== 0 || summary.dropped !== 0 || !(summary.paint >= (release ? 30 : 2))) {
  ...
  if (!receiver.some(row => row.kind === 'counters' && row.consumed >= (release ? 30 : 2))) reasons.push('Insufficient continuing receiver consumption');
...
export function validateResolumeEvidence(producer, receiver, outcome, initialCounters, release) {
  ...
  const ok = compiledEvidence(producer,release) && stable && outcome.code === 0 && summary?.paint >= (release ? 30 : 2) && summary?.closed === true && summary.failed === false && summary.webgpuReady === true &&
    summary.held === 0 && summary.uncertain === 0 && summary.dropped === 0 && consumed >= (release ? 30 : 2) &&
