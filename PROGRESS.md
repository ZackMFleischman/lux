...
| Installed hang detection | In progress | Detect a stuck visual even while its launcher still responds. | New worker heartbeat and 1.25 s deadline passed CPU checks; integration awaits the two fixes below. |
| Isolate failed-source cleanup | In progress | Stopping one failed export must not interrupt healthy exports. | Per-source asynchronous stop implemented; three new regressions pass in agent branch. Remaining tests and review underway. |
| Reject mixed runtime builds | In progress | Export must not mix new supervisor code with stale generated renderer files. | Add matching main/worker build markers and packaging rejection tests; native agent owns this. |
...
| Routine sampling and baseline mode | In progress | Reduce monitoring work and allow a fair monitoring-on/off comparison. | Collector modes and 1-in-30 GPU sampling implemented; 16 focused tests pass in agent branch. Worker/receiver wiring, tests and review remain. |
...
Stable checkout: `codex/tracer-0.1` at `a430544`. Compact controls, in-app confirmations, playback stability and functional Studio recovery are committed and tested. New installed-recovery and sampling changes remain in integration/agent branches pending review. Root coordinates graphics checks serially; native and performance agents work in parallel.
