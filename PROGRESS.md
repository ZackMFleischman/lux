...
| Recovered host image and cleanup | In progress | Restore the correct image/current controls within 5 s and verify resource cleanup. | Fresh test stopped the hung producer, but its 10 s total window left only 1.7 s for retry startup. No retry error recorded. Reviewing one longer observation within the existing 30 s outer limit; recovery pixels, five-second timing and GPU cleanup remain unverified. |
