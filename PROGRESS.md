...
| Additional declared-control host coverage | In progress | Verify a visual with several code-defined controls in the real host. | Five-control sphere source, saved values and QA instructions prepared. Final package will include the reviewed recovery fix; real-host checks remain. |
...
| Recovered host image and cleanup | In progress | Restore the correct image/current controls within 5 s and verify resource cleanup. | Fixed a Windows health-file lock that stopped automatic retry; independent review and 16 focused tests passed. Preparing a native check that recovery displays the latest control value. Exact five-second timing and GPU cleanup remain unverified. |
