...
| New-package Resolume workflow | In progress | Use new controls/assets, independent copies and saved compositions in Resolume. | Tested image package installed as Lux Image Tracer in the real source folder; manual transparency/save/reopen check requested. Native UI automation is unavailable here. Earlier legacy checks need not be repeated. |
...
| Physical stop and recovery timing | In progress | Confirm stuck execution actually exits within 2 s and correct host output returns within 5 s. | Agent implementing one bounded hang/physical-exit probe using existing Job lifecycle records. No hardware run yet. Full host-value recovery and GPU cleanup remain separate. |
...
Validated integration: `a800294`, including recovery scheduling, sampled monitoring and the native alpha/control fixture. The SDK example build/typecheck fix also passed root and version-specific typechecks. Root coordinates graphics checks serially; next are a bounded physical-stop test and the remaining actual Resolume checks. [Native alpha evidence](evidence/tracer-0.1/parameters-images-studio/installed-alpha.md).
