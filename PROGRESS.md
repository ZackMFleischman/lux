...
| Failure recovery | In progress | Broken visuals recover without trapping Lux or Resolume. | Real Studio error/hang recovery passed. Faster installed hang detection passed CPU checks; review found that stopping one failed source can stall healthy siblings, and that fix is underway. Physical stop/GPU cleanup and host-value recovery remain. |
