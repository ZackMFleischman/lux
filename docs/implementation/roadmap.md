...
**AI workflow follow-up: live source proposals (user feedback, 12 September).** After the first usable asset workflow, design genuine incremental AI source display in ordinary Source pane tabs. A clearly labeled read-only proposal has its own request identity and baseline draft version; streamed updates never mutate authoritative files, steal selection/focus, or rebuild on every chunk. Users can navigate and edit their working files concurrently. Complete candidate validation and the existing version guard govern atomic promotion; an intervening user edit causes a visible conflict instead of silent overwrite. Cancellation, disconnect, out-of-order updates, multi-file changes, explicit asset preservation, bounded buffering and undo require tests. Current MCP build receives complete arguments, so genuine live generation needs an incremental begin/update/finalize contract or an integration that can deliver model streaming events; do not simulate generation by slowly replaying a finished response. This is a moderate authoring feature, distinct from the later low-priority AI activity log, and does not block asset implementation.
...
flowchart LR
    ...
    Independent --> Life[0.2 host compatibility, lifecycle and source thumbnails]
