...
`node apps/studio/test-cpu.mjs` builds the Electron main/preload, browser renderer and test bundles, then runs 19 CPU-only checks. All passed. Three tests use React Testing Library and user-event in jsdom: disconnected inputs stay disabled while maximize/Escape works; play/pause and the MUI intensity slider send guarded shared operations without claiming applied state; command failure displays an actionable error and preserves the reported revision. jsdom starts no browser, GPU, Electron or canvas.
