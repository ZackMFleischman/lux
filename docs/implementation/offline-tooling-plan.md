...
| Declaration closure/parity | Existing trusted exact dependency versions; emitted staged pack; both SDKs and both Three imports | Generator manifest and package/notices hashes, positive semantic check, exact negative diagnostics, emitted/raw implementation identity comparison, successful-resolution inventory with no outside declaration. `--listFilesOnly` alone proves inventory only. |
