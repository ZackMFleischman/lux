...
| C7: offline performance validity/evaluation core | REVIEW FIXES IN PROGRESS | checkpoint_evaluator | Independent review reproduced four control/generation accounting edge cases; implementer is correcting them with regression fixtures before integration. Real hardware acceptance remains unavailable. |
| C8: required asset implementation contract | REVIEW CORRECTION | Coordinator | Design bf9da68 integrated as f362f87. Review requests explicit protection against user-modified JS intrinsics; correction and dependency-closure note added, awaiting recheck. Asset playback is not implemented yet. |
...
| C10: compact Studio UX audit | IN PROGRESS | compact_ui_audit | Audit current UI/code for per-group + pane picker, simpler Build/Open and file actions, compact accessible playback icons and spacing. Produces a prioritized design; no graphics launch or production changes. |
| C11: local pane picker and compact shell polish | QUEUED | Coordinator | Follow C10 findings; local picker must target its tab group, distinguish reopening/moving a singleton from new views, retain owners and provide empty-layout recovery. Broader multi-view state is routed to milestone 4. |
