...
| IN PROGRESS | Review and integrate export foundation | Coordinator + review lane | Package and registration integrated; caught and corrected custom install-root/native resolver mismatch. Installed-runtime integration still pending. |
...
| IN PROGRESS | Export action in Studio and cold-start/reopen QA | Coordinator | Preparing export backend and UI while native startup proceeds; final packaging depends on installed-runtime payload. Do not label package creation alone complete export. |
