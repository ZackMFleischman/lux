...
| IMPLEMENTED; PARTIAL HOST QA PASS | Shared installed runtime, automatic source startup, independent instances | Coordinator + native review | Watchdog155cb3b and nonblocking cleanup6705365 integrated; 14 CPU supervision tests and 11 native CTests pass. Packaged supervisor checks pass. User confirms two copies play independently, removal preserves the survivor, and process-cold reopen restores separate controls. Two different exported sources and measured acceptance remain pending. |
...
| PASS (BASIC MANUAL QA) | Installed export registration, playback and composition cold reopen | User + coordinator | Installation succeeds; two copies start/play with independent controls. After verified exit of Resolume/Lux processes, both reopen in roughly one second each and restore 0.2/0.8 with continued independence. Offline/checkout isolation, exact release telemetry and numerical acceptance remain separate pending gates. |
...
Final confirmation for this guided QA session: asked explicitly whether Intensity restored to 0.2/0.8 and changing one left the other unchanged, the user replied “yep that was fine.” Saved controls and duplicate independence after process-cold reopen therefore pass basic manual QA. The failed-drag blocker is resolved. Remaining work does not require repeating this basic sequence: prepare two distinct releases/asset coverage, offline/checkout-isolation validation, lifecycle/recovery failure evidence and instrumentation/performance gates; continue independent Studio layout work. Blank host thumbnails remain a scheduled usability issue.
