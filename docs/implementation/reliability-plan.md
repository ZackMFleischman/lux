...
Plan version 1, LUX-10 specification revision 2, inspected 13 September 2026 UTC by `/root/reliability_plan`, stable agent `8b1e7545-02c3-449b-8d45-a2451e5c81b0`, session `679835f9-243d-4aaa-a7b8-0774cbd13c0b`. Claim `177b40ed-4e02-4e81-ad2c-65ef144545a4` records the isolated checkout `C:/Users/zFlei/repos/lux/.worktrees/reliability-plan`, branch `codex/reliability-plan`, baseline `f95f3975892bf612bb63b94111893b18b84fff20`.
...
void runReceiverBody(Body body, OnFailure failure, Summary summary) {
  try { body(); }
  catch (const ReceiverStopRequested&) {} // Only the classified pending-copy stop.
  catch (const std::exception& error) { failure(error.what()); }
  catch (...) { failure("unknown worker exception"); }
  summary();
}
```
...
The helper must wrap the same main body currently guarded by the try/catches, not logging initialization before an instance exists. Diagnostic callbacks use the existing non-throwing stream configuration. A broken/open-failed stream remains unavailable; no summary can be promised on process kill, logging initialization failure, or a driver call that never returns. Exactly-once here means one finalization attempt after the body returns/throws through this boundary. Summary writes are not proof that GPU cleanup completed; they precede cleanup by design. Do not label a pending timing span complete or alter strict cadence rejection of genuine `failure` rows.
...
- [ ] **3. Implement the helper and use it in the actual receiver.** Add exactly the outcome policy above. Make summary the common finalization callback and preserve failure JSON (use existing diagnostic string escaping for error text). Verify source has no duplicate host-summary call and all body exceptions pass through the helper.
- [ ] **4. Exercise behavior through the shared seam.** Test normal return, dedicated stop, `std::runtime_error("local D3D copy deadline")`, `std::runtime_error("local D3D copy query failure")`, and `throw 7`. For each, assert summary count 1; failures respectively 0,0,1,1,1 with the expected reason. In the stop case, a sentinel statement after the throw must remain unexecuted. Capture callback order and assert failure precedes summary. A CPU fake callback is an integration seam used by production, not a second copy of its decision logic.
...
| Final host summary after body exits | Shared production boundary, normal/cancel/known/unknown failure injections; exactly one summary attempt with unchanged counts/loss. Failure precedes summary; no resumed body after cancellation. | CPU tests + review of FrameReceiver use; log availability limitations recorded. |
...
| Integrated source remains coherent | Current main recorded, isolated build of changed receiver, exact shared CMake hunk coordination, existing inspector rejection tests pass. | Root's landed SHA and combined focused test log; worker commit alone is not integration. |
