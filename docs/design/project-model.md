...
IDs are generated by core, opaque UUID strings, never names or list positions. A new renderer generation preserves logical `instanceId` but increments `generation`; reset increments `clockEpoch`. A frame identifies all three plus revision. Source hashes establish exact content; revision IDs establish history and concurrency, so identical bytes after Undo still have a new revision ID. `head: null` permits first creation through the same submit contract.
...
| --- | --- |
| `packages/core/src/contracts.ts` | IDs/snapshots/shared DTOs; schema rejects malformed IDs, paths, nonfinite values. |
