...
| --- | --- | --- |
| 0.1 | `apps/studio/src/TracerView.tsx`, `preview/PreviewSurface.tsx`, `preview/PreviewSession.ts`; one preview, named continuous control, transport/restart/status, fixed output dimensions. | AI create/capture/revise loop; invalid candidate retains output; independent UI/visual timings; closing Studio leaves host alive (T01–T09). |
| 0.1 early spike | `apps/studio/src/preview/preview-lifecycle.spike.ts`, `tests/integration/preview-lifecycle.spec.ts`; temporary attach/detach/popout harness before full shell. | Same instance/tick/controls through moves and failures; packaged origin/GPU/window gates above (U04). |
