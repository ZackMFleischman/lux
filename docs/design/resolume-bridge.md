...
For 0.1 use the fixed tracer control schema: one continuous float `Intensity`, stable ID `intensity`, native index 0, normalized 0–1. Under [DEC-13](../implementation/tracer-export-scope.md), export/install binds each named source to an immutable release; each loaded copy has a separate host-owned runtime. Authoring edits do not alter installed playback. Revisions must preserve the fixed schema or fail the tracer compatibility check; arbitrary schema publishing remains later.

Bring thin per-release source wrappers sharing one maintained bridge library into 0.1. Each wrapper freezes release identity and fixed schema in its binary/manifest; distinct releases need distinct host identities. Validate plugin-ID/registration constraints, complete package installation, automatic installed runtime startup and composition persistence in tracer. No general library editor or dynamic arbitrary-schema discovery is required. Packaging mechanism remains subject to actual-host validation; a single mutable global source cannot substitute for named reusable exports.

Each native plugin object creates a `pluginClientId` UUID; the service allocates and returns its canonical runtime-instance UUID on first Attach. Duplicate/reconnected Attach resolves that mapping, never blindly allocates again. Follow the [attachment and reconnect contract](tracer-contracts.md) for service epoch, connection epoch and stale-pipe rejection. A wrapper identifies immutable scene/release/schema hashes; saved host control indices are append-only within a compatible schema, never reassigned. Changed meaning/type/range or removed required controls requires a new incompatible release identity. Attachment/runtime UUIDs are not durable release identity. Two independent sources/copies and composition reopen are now 0.1 acceptance; broader compatibility cases remain 0.2.
...
See [runtime lifetime/watchdog](runtime.md) for leases, generations and restart. 0.1 must exercise two sources/copies, cold composition save/reopen and independent source removal; 0.2 extends resize/reactivation and compatibility coverage. Verify actual host persistence rather than assuming plugin-private arbitrary state is serialized. Wrapper identity plus host-native parameter persistence is the preferred minimal model. Cold start resolves installed releases and starts the background runtime without Studio or developer commands.
