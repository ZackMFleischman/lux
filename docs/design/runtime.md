...
Clients hold leases keyed by instance and client ID. A host-owned instance survives loss of every Studio lease. Under [DEC-13](../implementation/tracer-export-scope.md), 0.1 source wrappers resolve installed immutable releases and start the background runtime without Studio, including cold start. Each loaded source/copy has independent simulation and controls. Studio and host share immutable implementation bytes, never mutable state; authoring edits cannot retarget installed sources. Explicit new releases coexist with old ones. There is no implicit global singleton visual.
...
0.1 now includes independent host instances and installed composition persistence under DEC-13. 0.2 broadens lifecycle/resize/compatibility coverage without replacing `RuntimeKey`. 0.3 adds ordered timestamped events and replayable inputs. Milestone 2 adds a declared typed graph scheduler under `VisualInstance`: evaluate shared dependencies once, explicit previous-step feedback, linear premultiplied image boundaries and explicit numeric fields. Arbitrary JavaScript is not reverse-engineered into graph nodes.

Milestone 3 adds isolated controlled capture sessions. Milestone 4 adds audio/MIDI providers outside generated code and persisted creative mappings. Minimum pinned runtime/assets/control packaging is now tracer 0.1; milestone 5 hardens installation/distribution and covers the complete later feature set. Authoring compilation stays outside the host; installed playback requires no development compiler or source checkout.
