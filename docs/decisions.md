...
| DEC-10 | Tracer keeps a minimal visual API, one named continuous control and the actual AI image loop. DEC-13 supersedes its old single-source/export/installer exclusions. Full graph, durable revision history, embedded chat, library and audio UI remain later. | Original milestone boundaries, revised by explicit user decision DEC-13. |
...
| DEC-13 | Tracer must let the user create in Lux, export/install immutable reusable Resolume sources, and cold-start playback without Studio, AI, development tools or network. Bring two-source/duplicate-instance independence and composition reopen into 0.1. Include a basic install helper and pinned background runtime; retain full editor work and release hardening in later milestones. | Explicit user request, 12 September 2026. [Tracer export scope](implementation/tracer-export-scope.md) owns the revised minimum outcome and overrides earlier deferrals. This revision authorizes documentation changes only in this task. |
