...
function fixture(){
 ...
 return {expected:{releaseId,revisionId},experiment:{id:'run',child:{pid:42},timeoutMs:30000,outcome:'success',cleanupComplete:true,result:{cleanupComplete:true,exitCode:0,timeout:false,cancelled:false}},
...
test('held transport frames remain held; they are not fresh rendered-image claims',()=>{
 const x=fixture();for(const o of x.host.filter(x=>x.kind==='host-opportunity'))Object.assign(o,{present:true,generation:1,frameId:'5',copyCompletedQpc:'800'});
