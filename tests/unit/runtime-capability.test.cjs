...
test('new exports require matching liveness-v2 main, worker and relay while legacy checks remain compatible',()=>{
 ...
 const main='apps/render-host/src/main.cjs',worker='apps/render-host/src/compiled-worker.js',relay='apps/render-host/src/compiled-output.html';
 try{
  ...
  write(worker,'lux-worker-liveness-v2');write(relay,'old heartbeat relay');
  assert.throws(()=>assertRuntimeCapabilities(root,{workerLiveness:true}),/compiled-output.html lacks lux-relay-worker-liveness-v2/);
  write(relay,'lux-relay-worker-liveness-v2');assert.doesNotThrow(()=>assertRuntimeCapabilities(root,{workerLiveness:true}));
