...
test('pre-ready main-observed entry to independent empty-Job exit uses the unchanged two-second bound',()=>{
 const result=inspectNativeInitStop(fixture());assert.equal(result.mainObservedEntryToExitMs,1500);assert.equal(result.physicalExitVerified,true);assert.equal(result.exactHangOnsetMeasured,false);
});
test('missing exit, stale/mismatched identity, output readiness, loss, frequency or slow exit cannot pass',()=>{
 for(const mutate of [f=>f.lifecycle.pop(),f=>f.marker.ready=true,f=>f.marker.completedFrames=1,f=>f.marker.workerHeartbeat=0,
