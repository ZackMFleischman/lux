...
test('actual worker exposes verified assets during create and missing keys fail before readiness',async t=>{
  ...
  const oldUrl=URL.createObjectURL;
  const code=`export default {sdkVersion:'0.1.0',async create(context){
  ...
  for(const missing of [false,true]) {
    ...
    const body=linkedBody({...original,code,...assetFields});const linked={...body,linkedHash:hash(JSON.stringify(body))};
    ...
    (globalThis as any).onmessage({data:{type:'init',...identity,linked,settings:{width:1920,height:1080,fps:60,seed:0},controls:{intensity:0.5}}});
