...
export function refinementOverflow(ac, entropy, restart = false) {
  const codes = [1,...new Uint8Array(15),0];
  const firstScans = restart ? [0x7f,255,0xd0,0x7f] : [0x7f];
  return Buffer.concat([Buffer.from([255,0xd8]),segment(0xdb,[0,...new Uint8Array(64).fill(1)]),
    segment(0xc2,[8,0,8,0,restart ? 16 : 8,1,1,0x11,0]),segment(0xc4,[0,...codes,0x10,...codes]),
    ...(restart ? [segment(0xdd,[0,1])] : []),
    segment(0xda,[1,1,0,0,0,0]),Buffer.from(firstScans),segment(0xda,[1,1,0,1,63,1]),Buffer.from(firstScans),
    segment(0xc4,[0x10,1,...new Uint8Array(15),ac]),segment(0xda,[1,1,0,1,63,0x10]),
    Buffer.from(restart ? [...entropy,255,0xd0,...entropy] : entropy),Buffer.from([255,0xd9])]);
}
