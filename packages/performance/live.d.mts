export interface LiveMetric {
  ...
  readonly samplingPolicy:string;readonly validity:'complete'|'sampled'|'incomplete';readonly gate:'not_evaluated';
  readonly p50?:number;readonly p95?:number;readonly p99?:number;readonly max?:number;readonly value?:number;
}
export interface FrameSummary {
  readonly schemaVersion:1;readonly mode:'baseline'|'routine';readonly sequence:number;readonly clockDomain:'dedicated-worker-monotonic-ms';
...
export function createFrameCollector(options:{startMs:number;capacity?:number;windowMs?:number;mode?:'baseline'|'routine'}):Readonly<{
