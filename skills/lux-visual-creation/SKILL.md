...
- Parameters belong to each visual's code. Use the discovered declaration/control schema; Intensity is not a required global semantic. If this installation only exposes the tracer's fixed Intensity, explain that temporary product limitation briefly; keep meaningful named rebuild-only settings in source rather than silently claiming they are live controls. Continue the useful visual work without changing the SDK.
