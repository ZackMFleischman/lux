...
The entry must directly `export default defineVisual({...})`, importing `defineVisual` from `@lux/visual-sdk`. The checked version accepts `{ create }`; declaring a custom `controls` field is a type error. Discovery's `contractSource` and `sharedContractSource` expose the current types.
...
The contract includes `context.assets`, but **the checked discovery reports `assetPlayback: false`**. Source v2 can retain bounded opaque 24-bit BMP records, yet this does not mean images render in the runtime. PNG/JPEG and alpha support are being implemented separately. Check discovery before using them; never remove existing assets because the current task is procedural. When playback is supported, read that installation's exposed asset/texture API rather than assuming bytes are a ready-to-use Three Texture.
